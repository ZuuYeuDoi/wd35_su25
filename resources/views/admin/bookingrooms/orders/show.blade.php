...
<section role="main" class="content-body">
    ...
    <form action="{{ route('room_order.update', $booking->id) }}" method="POST" class="order-details action-buttons-fixed">
        @csrf
        @method('PUT')
                        ...
                        <div class="form-row">
                            <div class="form-group col mb-3">
                                ...
                                <select class="form-control" name="status" required>
                                    <option value="0" {{ $booking->status == 0 ? 'selected' : '' }}>Chưa xác nhận</option>
                                    <option value="1" {{ $booking->status == 1 ? 'selected' : '' }}>Đã xác nhận</option>
                                    <option value="2" {{ $booking->status == 2 ? 'selected' : '' }}>Hoàn tất</option>
...
" readonly />
                                    ...
                                    <div class="time">
                                        ...
                                        <input type="text" class="form-control form-control-modern text-center"
                                            name="orderTimeHour" value="{{ $booking->created_at ? $booking->created_at->format('H') : '' }}" readonly />
                                        <span class="px-2">:</span>
                                        <input type="text" class="form-control form-control-modern text-center"
                                            name="orderTimeMin" value="{{ $booking->created_at ? $booking->created_at->format('i') : '' }}" readonly />
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-row">
                            <div class="form-group col mb-3">
                                ...
                                <input type="text" class="form-control form-control-modern" value="{{ $booking->guest->name ?? '---' }}" readonly>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-xl-6 mb-4">
                                <h3 class="text-color-dark font-weight-bold text-4 line-height-1 mt-0 mb-3">KHÁCH HÀNG</h3>
                                <ul class="list list-unstyled list-item-bottom-space-0">
                                    <li>{{ $booking->guest->address ?? '---' }}</li>
                                    <li>{{ $booking->guest->city ?? '' }}</li>
                                    <li>{{ $booking->guest->country ?? '' }}</li>
                                </ul>
                                ...
                                <a href="tel:{{ $booking->guest->phone }}" class="text-color-dark">{{ $booking->guest->phone }}</a>
                            </div>

                            <!-- Nhận phòng -->
                            <div class="col-xl-6">
                                <h3 class="font-weight-bold text-color-dark text-4 line-height-1 mt-0 mb-3">NHẬN PHÒNG</h3>
                                <ul class="list list-unstyled list-item-bottom-space-0">
                                    <li>Ngày nhận: {{ $booking->check_in_date }}</li>
                                    <li>Ngày trả: {{ $booking->check_out_date }}</li>
                                </ul>
                                <strong class="d-block text-color-dark">Ghi chú:</strong>
                    ...
                    <div class="card-body">
                        ...
                        <div class="row justify-content-end mt-4">
                            <div class="col-auto me-5">
                                ...
                                <strong class="text-color-dark text-5">{{ number_format($booking->room->price) }}đ</strong>
                            </div>
                        </div>
                    </div>
                </div>
